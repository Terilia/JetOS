...
// HollowPlanet — 150 km hollow world disguised as a normal Earth, generated by a PROCEDURAL VOXEL PROVIDER
// so SE's clipmap renderer streams LODs to the horizon, exactly like a real planet.
namespace HollowPlanet
{
    // ---- shared procedural definition of the inverted Earth shell ----
    internal static class Hollow
    {
        public const int SIZE = 262144;            // storage edge (>= 150 km), power of two
        public const double HALF = SIZE / 2;       // shell centre, in storage voxel coords
        ...
        public const double FREQ = 40.0;           // terrain feature frequency on the sphere
        public const float GRAV = 9.81f;
        public const int TYPE_ID = 770077;
    ...
    // ---- the procedural voxel provider (SE asks it for terrain at any LOD) ----
    [MyStorageDataProvider(Hollow.TYPE_ID)]
    public class HollowEarthProvider : IMyStorageDataProvider
    {
        ...
        {
            var req = new MyVoxelDataRequest { Target = target, Offset = writeOffset, Lod = lodIndex, MinInLod = minInLod, MaxInLod = maxInLod, RequestedData = dataType };
            ReadRange(ref req);
        }
        ...
        // min/max distance from the shell centre over the LOD box (fast empty/space rejection)
        static void RadialBounds(int lvs, ref Vector3I min, ref Vector3I max, out double rmin, out double rmax)
        {
            double loX = min.X * (double)lvs - Hollow.HALF, hiX = (max.X + 1) * (double)lvs - Hollow.HALF;
            double loY = min.Y * (double)lvs - Hollow.HALF, hiY = (max.Y + 1) * (double)lvs - Hollow.HALF;
            double loZ = min.Z * (double)lvs - Hollow.HALF, hiZ = (max.Z + 1) * (double)lvs - Hollow.HALF;
        ...
        MyVoxelRequestFlags ReadContent(MyStorageData target, ref Vector3I off, int lod, ref Vector3I min, ref Vector3I max, bool detectOnly)
        {
            ...
            double bandLo = Hollow.R - Hollow.AMP, bandHi = Hollow.R + Hollow.THICK + Hollow.AMP, margin = lvs * 2;
            ...
            for (c.Z = min.Z; c.Z <= max.Z; c.Z++)
                for (c.Y = min.Y; c.Y <= max.Y; c.Y++)
                    ...
                    {
                        double sx = c.X * (double)lvs - Hollow.HALF, sy = c.Y * (double)lvs - Hollow.HALF, sz = c.Z * (double)lvs - Hollow.HALF;
                        double r = Math.Sqrt(sx * sx + sy * sy + sz * sz);
                        double inv = r > 1e-6 ? 1.0 / r : 0.0;
                        double surf = Hollow.SurfaceR(sx * inv, sy * inv, sz * inv);
                        double depth = r - surf;
                        double d = depth < 0 ? -depth : (depth > Hollow.THICK ? depth - Hollow.THICK : -Math.Min(depth, Hollow.THICK - depth));
                        p = off + (c - min);
                        byte content = SdToContent(d / lvs);
        ...
        {
            double field = (((x * 73856093) ^ (y * 19349663) ^ (z * 83492791)) & 0x7fffffff) / 2147483647.0;
            if (Hollow.Hash(x >> 4, y >> 4, z >> 4) < 0.80) return _stone;
            double pick = Hollow.Hash(x >> 5, y >> 5, z >> 5);
    ...
    // ---- real, detected outward natural gravity ----
    ...
    public class HollowPlanetSession : MySessionComponentBase
    {
        bool _done, _failed;
        public override void UpdateBeforeSimulation()
        {
            if (_done || _failed || MyAPIGateway.Session == null || !MyAPIGateway.Multiplayer.IsServer) return;
            try
            {
                if (MyAPIGateway.GravityProviderSystem == null) return;
                MyAPIGateway.GravityProviderSystem.AddNaturalModAPI(Vector3D.Zero,
                    new OutwardGravity { Center = Vector3D.Zero, Accel = Hollow.GRAV, Limit = Hollow.R + Hollow.AMP + Hollow.THICK + 500.0 });
                _done = true;
            }
            catch (Exception e) { _failed = true; MyAPIGateway.Utilities.ShowMessage("HollowPlanet", "err: " + e.Message); }
        }
    }
}
